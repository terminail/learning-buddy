@echo off
REM Test runner script for nested statements exercise

REM Build first if needed
if not exist "build\test_nested_statements_basics.exe" if not exist "build\test_nested_statements_practice.exe" (
...
echo Running all nested statements tests...
test_nested_statements_basics.exe > basics_output.txt 2>&1
test_nested_statements_practice.exe > practice_output.txt 2>&1
