@echo off
REM Test runner script for loop statements exercise

REM Build first if needed
if not exist "build\test_loop_statements_basics.exe" if not exist "build\test_loop_statements_practice.exe" (
...
echo Running all loop statements tests...
test_loop_statements_basics.exe > basics_output.txt 2>&1
test_loop_statements_practice.exe > practice_output.txt 2>&1
