@echo off
REM Test runner script for stream states exercise

REM Build first if needed
if not exist "build\test_stream_states_basics.exe" if not exist "build\test_stream_states_practice.exe" (
...
test_stream_states_basics.exe > basics_output.txt 2>&1
test_stream_states_practice.exe > practice_output.txt 2>&1
