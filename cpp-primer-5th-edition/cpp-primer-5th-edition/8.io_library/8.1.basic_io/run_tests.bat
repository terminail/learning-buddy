@echo off
REM Test runner script for basic IO exercise

REM Build first if needed
if not exist "build\test_basic_io_basics.exe" if not exist "build\test_basic_io_practice.exe" (
...
test_basic_io_basics.exe > basics_output.txt 2>&1
test_basic_io_practice.exe > practice_output.txt 2>&1
