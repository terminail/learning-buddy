@echo off
REM Test runner script for file streams exercise

REM Build first if needed
if not exist "build\test_file_streams_basics.exe" if not exist "build\test_file_streams_practice.exe" (
...
test_file_streams_basics.exe > basics_output.txt 2>&1
test_file_streams_practice.exe > practice_output.txt 2>&1
