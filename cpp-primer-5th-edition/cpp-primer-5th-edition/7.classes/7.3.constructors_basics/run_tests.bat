@echo off
REM Test runner script for constructors basics exercise

REM Build first if needed
if not exist "build\test_constructors_basics.exe" if not exist "build\test_constructors_practice.exe" (
...
test_constructors_basics.exe > basics_output.txt 2>&1
test_constructors_practice.exe > practice_output.txt 2>&1
