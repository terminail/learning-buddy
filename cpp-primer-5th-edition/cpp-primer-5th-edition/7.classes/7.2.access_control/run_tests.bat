@echo off
REM Test runner script for access control exercise

REM Build first if needed
if not exist "build\test_access_control_basics.exe" if not exist "build\test_access_control_practice.exe" (
...
test_access_control_basics.exe > basics_output.txt 2>&1
test_access_control_practice.exe > practice_output.txt 2>&1
