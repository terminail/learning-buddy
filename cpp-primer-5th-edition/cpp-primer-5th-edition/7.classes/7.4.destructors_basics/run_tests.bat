@echo off
REM Test runner script for destructors basics exercise

REM Build first if needed
if not exist "build\test_destructors_basics.exe" if not exist "build\test_destructors_practice.exe" (
...
test_destructors_basics.exe > basics_output.txt 2>&1
test_destructors_practice.exe > practice_output.txt 2>&1
