@echo off
REM Test runner script for hello_world exercise (Windows)

REM Build first if needed
if not exist "build\test_hello_world_basics.exe" if not exist "build\test_hello_world_practice.exe" (
...
test_hello_world_basics.exe > basics_output.txt 2>&1
test_hello_world_practice.exe > practice_output.txt 2>&1
