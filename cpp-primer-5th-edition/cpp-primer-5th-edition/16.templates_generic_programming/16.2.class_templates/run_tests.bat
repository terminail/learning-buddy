@echo off
REM Test runner script for class_templates exercise

REM Build first if needed
if not exist "build\test_class_templates_basics.exe" if not exist "build\test_class_templates_practice.exe" (
...
echo Running all class templates tests...
test_class_templates_basics.exe > basics_output.txt 2>&1
test_class_templates_practice.exe > practice_output.txt 2>&1
