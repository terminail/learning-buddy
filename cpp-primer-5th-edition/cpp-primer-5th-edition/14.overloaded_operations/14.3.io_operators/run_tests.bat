@echo off
REM Test runner script for IO operators exercise

REM Build first if needed
if not exist "build\test_io_operators_basics.exe" if not exist "build\test_io_operators_practice.exe" (
...
test_io_operators_basics.exe > basics_output.txt 2>&1
test_io_operators_practice.exe > practice_output.txt 2>&1
