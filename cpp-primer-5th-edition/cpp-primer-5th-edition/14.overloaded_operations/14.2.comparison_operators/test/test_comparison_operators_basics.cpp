...
TEST(ComparisonOperatorsTest, Relational) {
    ...
    TestStudent s4("Alice", 3.5); // Same as s1
    
    EXPECT_TRUE(s1 < s2);
    EXPECT_TRUE(s2 > s1);
    EXPECT_TRUE(s1 <= s2);
    EXPECT_TRUE(s2 >= s1);
    EXPECT_TRUE(s1 <= s3);  // s1 < s3, so s1 <= s3 should be true
    EXPECT_FALSE(s1 >= s3); // s1 < s3, so s1 >= s3 should be false
    EXPECT_TRUE(s1 <= s4);  // s1 == s4, so s1 <= s4 should be true
    EXPECT_TRUE(s1 >= s4);  // s1 == s4, so s1 >= s4 should be true
}
