@echo off
REM Test runner script for string_algorithms exercise

REM Build first if needed
if not exist "build\test_string_algorithms_basics.exe" if not exist "build\test_string_algorithms_practice.exe" (
...
test_string_algorithms_basics.exe > basics_output.txt 2>&1
test_string_algorithms_practice.exe > practice_output.txt 2>&1
