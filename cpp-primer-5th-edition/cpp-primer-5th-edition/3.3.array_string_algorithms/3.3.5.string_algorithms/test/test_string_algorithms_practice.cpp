...
TEST(StringAlgorithmsPractice, AdvancedProcessing) {
    ...
    // Find the position of 'W' in "Welcome" - it should be after "Hello, World! "
    // "Hello, World! " is 14 characters, so 'W' should be at position 14
    EXPECT_EQ('W', text[14]); // 'W' in "Welcome"
    ...
    // After removing words shorter than 5 characters:
    // "date" (4 characters) should be removed
    // "apple" (5 characters), "banana" (6 characters), "cherry" (6 characters), "elderberry" (10 characters) should remain
    EXPECT_EQ(4, words.size());
    EXPECT_EQ("apple", words[0]);
    EXPECT_EQ("banana", words[1]);
    EXPECT_EQ("cherry", words[2]);
    EXPECT_EQ("elderberry", words[3]);
    ...
    EXPECT_EQ("cherry", words[2]);     // 6 characters
    EXPECT_EQ("elderberry", words[3]); // 10 characters
}
