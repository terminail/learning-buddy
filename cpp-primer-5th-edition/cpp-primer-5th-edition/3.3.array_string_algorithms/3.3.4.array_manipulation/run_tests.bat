@echo off
REM Test runner script for array_manipulation exercise

REM Build first if needed
if not exist "build\test_array_manipulation_basics.exe" if not exist "build\test_array_manipulation_practice.exe" (
...
test_array_manipulation_basics.exe > basics_output.txt 2>&1
test_array_manipulation_practice.exe > practice_output.txt 2>&1
