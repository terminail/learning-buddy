...
#include <algorithm>
#include <cctype>
#include <functional>  // Required for std::boyer_moore_searcher
...
int main() {
    ...
    // Search for substring using standard std::search
    std::string pattern = "World";
    auto str_it = std::search(text.begin(), text.end(), pattern.begin(), pattern.end());
    if (str_it != text.end()) {
        std::cout << "Found 'World' at position: " << (str_it - text.begin()) << std::endl;
    } else {
        std::cout << "'World' not found in text" << std::endl;
    }
    
    // Find end (last occurrence)
    std::string char_pattern = "o";
    auto end_it = std::find_end(text.begin(), text.end(), char_pattern.begin(), char_pattern.end());
    if (end_it != text.end()) {
        std::cout << "Last 'o' at position: " << (end_it - text.begin()) << std::endl;
    } else {
        std::cout << "'o' not found in text" << std::endl;
    }
