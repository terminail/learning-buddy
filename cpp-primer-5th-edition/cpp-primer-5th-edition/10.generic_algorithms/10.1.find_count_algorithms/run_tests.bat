@echo off
REM Test runner script for find_count_algorithms exercise

REM Build first if needed
if not exist "build\test_find_count_algorithms_basics.exe" if not exist "build\test_find_count_algorithms_practice.exe" (
...
echo Running all find count algorithms tests...
test_find_count_algorithms_basics.exe > basics_output.txt 2>&1
test_find_count_algorithms_practice.exe > practice_output.txt 2>&1
