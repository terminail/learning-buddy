@echo off
REM Test runner script for merge algorithms exercise

REM Build first if needed
if not exist "build\test_merge_algorithms_basics.exe" if not exist "build\test_merge_algorithms_practice.exe" (
...
echo Running all merge algorithms tests...
test_merge_algorithms_basics.exe > basics_output.txt 2>&1
test_merge_algorithms_practice.exe > practice_output.txt 2>&1
