@echo off
REM Test runner script for move semantics basics exercise

REM Build first if needed
if not exist "build\test_move_semantics_basics_basics.exe" if not exist "build\test_move_semantics_basics_practice.exe" (
...
echo Running all move semantics basics tests...
test_move_semantics_basics_basics.exe > basics_output.txt 2>&1
test_move_semantics_basics_practice.exe > practice_output.txt 2>&1
