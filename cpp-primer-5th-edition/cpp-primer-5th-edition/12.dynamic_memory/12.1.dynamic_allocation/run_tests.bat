@echo off
REM Test runner script for dynamic_allocation exercise

REM Build first if needed
if not exist "build\test_dynamic_allocation_basics.exe" if not exist "build\test_dynamic_allocation_practice.exe" (
...
test_dynamic_allocation_basics.exe > basics_output.txt 2>&1
test_dynamic_allocation_practice.exe > practice_output.txt 2>&1
