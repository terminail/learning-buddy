@echo off
REM Test runner script for smart_pointers exercise

REM Build first if needed
if not exist "build\test_smart_pointers_basics.exe" if not exist "build\test_smart_pointers_practice.exe" (
...
test_smart_pointers_basics.exe > basics_output.txt 2>&1
test_smart_pointers_practice.exe > practice_output.txt 2>&1
