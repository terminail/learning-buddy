@echo off
REM Test runner script for function parameters exercise

REM Build first if needed
if not exist "build\test_function_parameters_basics.exe" if not exist "build\test_function_parameters_practice.exe" (
...
test_function_parameters_basics.exe > basics_output.txt 2>&1
test_function_parameters_practice.exe > practice_output.txt 2>&1
