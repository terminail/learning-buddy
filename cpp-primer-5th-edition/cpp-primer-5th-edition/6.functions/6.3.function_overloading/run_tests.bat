@echo off
REM Test runner script for function overloading exercise

REM Build first if needed
if not exist "build\test_function_overloading_basics.exe" if not exist "build\test_function_overloading_practice.exe" (
...
test_function_overloading_basics.exe > basics_output.txt 2>&1
test_function_overloading_practice.exe > practice_output.txt 2>&1
