@echo off
REM Test runner script for basic functions exercise

REM Build first if needed
if not exist "build\test_basic_functions_basics.exe" if not exist "build\test_basic_functions_practice.exe" (
...
echo Running all basic functions tests...
test_basic_functions_basics.exe > basics_output.txt 2>&1
test_basic_functions_practice.exe > practice_output.txt 2>&1
