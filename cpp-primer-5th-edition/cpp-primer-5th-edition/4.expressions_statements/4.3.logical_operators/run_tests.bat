@echo off
REM Test runner script for logical_operators exercise

REM Build first if needed
if not exist "build\test_logical_operators_basics.exe" if not exist "build\test_logical_operators_practice.exe" (
...
test_logical_operators_basics.exe > basics_output.txt 2>&1
test_logical_operators_practice.exe > practice_output.txt 2>&1
