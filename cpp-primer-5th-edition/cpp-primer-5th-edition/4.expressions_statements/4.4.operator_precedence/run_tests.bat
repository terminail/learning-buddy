@echo off
REM Test runner script for operator_precedence exercise

REM Build first if needed
if not exist "build\test_operator_precedence_basics.exe" if not exist "build\test_operator_precedence_practice.exe" (
...
test_operator_precedence_basics.exe > basics_output.txt 2>&1
test_operator_precedence_practice.exe > practice_output.txt 2>&1
