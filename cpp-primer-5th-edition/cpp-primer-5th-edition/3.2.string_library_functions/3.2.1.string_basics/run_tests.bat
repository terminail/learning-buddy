@echo off
REM Test runner script for string_basics exercise

REM Build first if needed
if not exist "build\test_string_basics_basics.exe" if not exist "build\test_string_basics_practice.exe" (
...
test_string_basics_basics.exe > basics_output.txt 2>&1
test_string_basics_practice.exe > practice_output.txt 2>&1
