@echo off
REM Test runner script for random_numbers exercise

REM Build first if needed
if not exist "build\test_random_numbers_basics.exe" if not exist "build\test_random_numbers_practice.exe" (
...
test_random_numbers_basics.exe > basics_output.txt 2>&1
test_random_numbers_practice.exe > practice_output.txt 2>&1
