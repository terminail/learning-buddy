@echo off
REM Test runner script for bitset_operations exercise

REM Build first if needed
if not exist "build\test_bitset_operations_basics.exe" if not exist "build\test_bitset_operations_practice.exe" (
...
test_bitset_operations_basics.exe > basics_output.txt 2>&1
test_bitset_operations_practice.exe > practice_output.txt 2>&1
