@echo off
REM Test runner script for namespaces exercise

REM Build first if needed
if not exist "build\test_namespaces_basics.exe" if not exist "build\test_namespaces_practice.exe" (
...
test_namespaces_basics.exe > basics_output.txt 2>&1
test_namespaces_practice.exe > practice_output.txt 2>&1
