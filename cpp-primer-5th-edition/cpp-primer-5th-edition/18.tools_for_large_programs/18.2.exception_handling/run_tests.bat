@echo off
REM Test runner script for exception handling exercise

REM Build first if needed
if not exist "build\test_exception_handling_basics.exe" if not exist "build\test_exception_handling_practice.exe" (
...
test_exception_handling_basics.exe > basics_output.txt 2>&1
test_exception_handling_practice.exe > practice_output.txt 2>&1
