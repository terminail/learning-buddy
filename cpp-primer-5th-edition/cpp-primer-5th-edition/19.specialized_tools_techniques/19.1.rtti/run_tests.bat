@echo off
REM Test runner script for RTTI exercise

REM Build first if needed
if not exist "build\test_rtti_basics.exe" if not exist "build\test_rtti_practice.exe" (
...
test_rtti_basics.exe > basics_output.txt 2>&1
test_rtti_practice.exe > practice_output.txt 2>&1
