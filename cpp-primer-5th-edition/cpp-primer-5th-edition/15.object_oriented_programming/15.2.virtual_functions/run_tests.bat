@echo off
REM Test runner script for virtual_functions exercise

REM Build first if needed
if not exist "build\test_virtual_functions_basics.exe" if not exist "build\test_virtual_functions_practice.exe" (
...
test_virtual_functions_basics.exe > basics_output.txt 2>&1
test_virtual_functions_practice.exe > practice_output.txt 2>&1
