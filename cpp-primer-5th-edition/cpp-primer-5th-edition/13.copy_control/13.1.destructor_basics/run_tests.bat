@echo off
REM Test runner script for destructor basics exercise

REM Build first if needed
if not exist "build\test_destructor_basics_basics.exe" if not exist "build\test_destructor_basics_practice.exe" (
...
echo Running all destructor basics tests...
test_destructor_basics_basics.exe > basics_output.txt 2>&1
test_destructor_basics_practice.exe > practice_output.txt 2>&1
