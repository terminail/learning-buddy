@echo off
REM Test runner script for array basics exercise (Windows)

REM Build first if needed
if not exist "build\test_array_basics_basics.exe" if not exist "build\test_array_basics_practice.exe" (
...
echo Running all array basics tests...
test_array_basics_basics.exe > basics_output.txt 2>&1
test_array_basics_practice.exe > practice_output.txt 2>&1
