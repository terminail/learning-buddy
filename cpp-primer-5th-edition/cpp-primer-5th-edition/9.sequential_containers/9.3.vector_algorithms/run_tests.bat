@echo off
REM Test runner script for vector algorithms exercise

REM Build first if needed
if not exist "build\test_vector_algorithms_basics.exe" if not exist "build\test_vector_algorithms_practice.exe" (
...
echo Running all vector algorithms tests...
test_vector_algorithms_basics.exe > basics_output.txt 2>&1
test_vector_algorithms_practice.exe > practice_output.txt 2>&1
