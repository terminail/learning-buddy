@echo off
REM Test runner script for vector operations exercise

REM Build first if needed
if not exist "build\test_vector_operations_basics.exe" if not exist "build\test_vector_operations_practice.exe" (
...
echo Running all vector operations tests...
test_vector_operations_basics.exe > basics_output.txt 2>&1
test_vector_operations_practice.exe > practice_output.txt 2>&1
