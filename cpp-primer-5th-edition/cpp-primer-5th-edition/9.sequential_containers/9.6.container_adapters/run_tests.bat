@echo off
REM Test runner script for container adapters exercise

REM Build first if needed
if not exist "build\test_container_adapters_basics.exe" if not exist "build\test_container_adapters_practice.exe" (
...
test_container_adapters_basics.exe > basics_output.txt 2>&1
test_container_adapters_practice.exe > practice_output.txt 2>&1
