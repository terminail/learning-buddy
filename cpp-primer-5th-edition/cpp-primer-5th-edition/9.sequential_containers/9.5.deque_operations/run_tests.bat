@echo off
REM Test runner script for deque operations exercise

REM Build first if needed
if not exist "build\test_deque_operations_basics.exe" if not exist "build\test_deque_operations_practice.exe" (
...
test_deque_operations_basics.exe > basics_output.txt 2>&1
test_deque_operations_practice.exe > practice_output.txt 2>&1
