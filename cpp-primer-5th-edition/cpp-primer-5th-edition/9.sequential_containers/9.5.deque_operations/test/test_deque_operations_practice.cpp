...
TEST(DequeOperationsPractice, AdvancedOperations) {
    ...
    // Insert multiple elements at position 2
    numbers.insert(numbers.begin() + 2, {99, 88, 77});
    // Result: {1, 2, 99, 88, 77, 3, 4, 5}
    EXPECT_EQ(8, numbers.size());
    EXPECT_EQ(99, numbers[2]);
    EXPECT_EQ(88, numbers[3]);
    EXPECT_EQ(77, numbers[4]);
    
    // Erase elements from position 1 to 4 (exclusive)
    // This removes elements at positions 1, 2, 3
    // Which are: 2, 99, 88
    // Result should be: {1, 77, 3, 4, 5}
    numbers.erase(numbers.begin() + 1, numbers.begin() + 4);
    EXPECT_EQ(5, numbers.size());
    EXPECT_EQ(1, numbers[0]);
    EXPECT_EQ(77, numbers[1]);  // This was at index 4 before erase
    EXPECT_EQ(3, numbers[2]);   // This was at index 5 before erase
