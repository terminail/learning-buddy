@echo off
REM Test runner script for list operations exercise

REM Build first if needed
if not exist "build\test_list_basics_basics.exe" if not exist "build\test_list_basics_practice.exe" (
...
echo Running all list operations tests...
test_list_basics_basics.exe > basics_output.txt 2>&1
test_list_basics_practice.exe > practice_output.txt 2>&1
