@echo off
REM Test runner script for pair multiset exercise

REM Build first if needed
if not exist "build\test_pair_multiset_basics.exe" if not exist "build\test_pair_multiset_practice.exe" (
...
test_pair_multiset_basics.exe > basics_output.txt 2>&1
test_pair_multiset_practice.exe > practice_output.txt 2>&1
