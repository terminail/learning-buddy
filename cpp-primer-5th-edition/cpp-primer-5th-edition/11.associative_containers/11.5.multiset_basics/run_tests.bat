@echo off
REM Test runner script for multiset basics exercise

REM Build first if needed
if not exist "build\test_multiset_basics_basics.exe" if not exist "build\test_multiset_basics_practice.exe" (
...
echo Running all multiset basics tests...
test_multiset_basics_basics.exe > basics_output.txt 2>&1
test_multiset_basics_practice.exe > practice_output.txt 2>&1
