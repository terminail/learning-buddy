@echo off
REM Test runner script for pair multimap exercise

REM Build first if needed
if not exist "build\test_pair_multimap_basics.exe" if not exist "build\test_pair_multimap_practice.exe" (
...
echo Running all pair multimap tests...
test_pair_multimap_basics.exe > basics_output.txt 2>&1
test_pair_multimap_practice.exe > practice_output.txt 2>&1
